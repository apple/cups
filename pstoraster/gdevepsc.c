...
{	static char graphics_modes_9[5] =
	...
	byte *in = (byte *)gs_malloc(in_size, 1, "epsc_print_page(in)");
	int out_size = ((pdev->width + 7) & -8) * y_mult;
	byte *out = (byte *)gs_malloc(out_size, 1, "epsc_print_page(out)");
...
/* declare color buffer and related vars */
	...
	/* Check allocations */
	if ( in == 0 || out == 0 )
	   {	if ( in ) gs_free((char *)in, in_size, 1, "epsc_print_page(in)");
		if ( out ) gs_free((char *)out, out_size, 1, "epsc_print_page(out)");
		return -1;
	   }
...
/*	Create color buffer */
	if (gx_device_has_color(pdev))
		...
		if((color_in = (byte *)gs_malloc(color_in_size, 1,
			...
			{
			gs_free((char *)in, in_size, 1, "epsc_print_page(in)");
			gs_free((char *)out, out_size, 1, "epsc_print_page(out)");
			return(-1);
			}
		}
	else
...
*/
	...
	gs_free((char *)in, in_size, 1, "epsc_print_page(in)");
	if (gx_device_has_color(pdev))
		gs_free((char *)color_in, color_in_size, 1, "epsc_print_page(rin)");
	return 0;
}
