  ...
  */
            ...
            commandinfo.st_uid))
  {
    *pid = 0;
    ...
    cupsdLogMessage(CUPSD_LOG_ERROR,
                    "Unable to execute \"%s\": insecure file permissions "
	...
	cupsdAddEvent(CUPSD_EVENT_PRINTER_STATE, job->printer, NULL,
		      "Printer driver \"%s\" has insecure file permissions "
		      "(%d/0%o).", command, (int)commandinfo.st_uid,
		      commandinfo.st_mode);
    }
           ...
           !(commandinfo.st_mode & S_IXOTH))
  {
    *pid = 0;
    ...
    cupsdLogMessage(CUPSD_LOG_DEBUG2,
		    ...
		    root, profile, job, job ? job->id : 0, pid, *pid);
    cupsdLogMessage(CUPSD_LOG_ERROR,
                    "Unable to execute \"%s\": no execute permissions (%d/0%o)",
  ...
  else if (!RunUser && (commandinfo.st_mode & S_IWGRP))
  {
    cupsdLogMessage(CUPSD_LOG_WARN,
                    "\"%s\": insecure file permissions (%d/0%o)", command,
