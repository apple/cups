/*
 * "$Id: language.c,v 1.17 2000/11/02 22:19:22 mike Exp $"
 *
...
cupsLangGet(const char *language) /* I - Language or locale */
{
  ...
  */
  ...
  snprintf(filename, sizeof(filename) - 1, "%s/%s/cups_%s", localedir, real, real);
  ...
  if ((fp = fopen(filename, "r")) == NULL)
    if (strlen(real) > 2)
    {
      ...
      */
      ...
      real[2] = '\0';
      snprintf(filename, sizeof(filename) - 1, "%s/%s/cups_%s", localedir, real,
               real);
      fp = fopen(filename, "r");
    }
...
/*
 * End of "$Id: language.c,v 1.17 2000/11/02 22:19:22 mike Exp $".
 */
